...
class P4P7Setup:
    ...
    def setup_p5_precommit(self):
        ...
        try:
            ...
            os.chdir(self.repo_root)
            result = subprocess.run(
                ["pre-commit", "install"], capture_output=True, text=True, timeout=30
            )
    ...
    def run_all_checks(self):
        ...
        print("SUMMARY")
        print("=" * 70)
        p4_status = "✅ COMPLETE" if results.get("p4", False) else "⚠️  PARTIAL"
        print(f"P4 CAD Functions:      {p4_status}")
        p4_gen_status = (
            "✅ COMPLETE" if results.get("p4_gen", False) else "⚠️  NEEDS TEST"
        )
        print(f"P4 CAD Generation:     {p4_gen_status}")
        p5_status = "✅ COMPLETE" if results.get("p5", False) else "⚠️  NEEDS SETUP"
        print(f"P5 Pre-Commit Hooks:   {p5_status}")
        p6_status = "✅ COMPLETE" if results.get("p6", False) else "⚠️  INCOMPLETE"
        print(f"P6 Documentation:      {p6_status}")
        p7_status = "✅ COMPLETE" if results.get("p7", False) else "⚠️  PARTIAL"
        print(f"P7 Hardware Emulation: {p7_status}")
        print("=" * 70)
...
def main():
    ...
    parser.add_argument(
        "--check-p4", action="store_true", help="Check P4 CAD functions"
    )
    parser.add_argument(
        "--setup-p5", action="store_true", help="Setup P5 pre-commit hooks"
    )
    parser.add_argument(
        "--check-p6", action="store_true", help="Check P6 documentation"
    )
    parser.add_argument(
        "--check-p7", action="store_true", help="Check P7 hardware emulation"
    )
    parser.add_argument("--all", action="store_true", help="Run all checks and setup")
