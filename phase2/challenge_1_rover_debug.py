...
Your task is to identify and fix logic errors in movement,
rotation, and state reporting.
"""
...
class Rover:
    ...
    def get_status(self):
        """Return the rover's current status."""
        status = (
            f"Rover at ({self.x}, {self.y}) "
            f"facing {self.direction} "
            f"with {self.battery}% battery"
        )
        return status
