---
name: Program Enrollment Inquiry
about: Apply to Detroit Automation Academy programs
title: '[ENROLLMENT] Your Name - Program Interest'
labels: 'enrollment'
assignees: ''
---

## 📋 Quick Instructions
Thank you for your interest in the Detroit Automation Academy! This form takes approximately **2-3 minutes** to complete. We'll review your information and reach out within 5 business days.

---

## 👤 Personal Information *(Required)*

### Full Name *
[Enter your full name here]

### Email Address *
[your.email@example.com]

### Phone Number *
[Your phone number, e.g., (313) 555-1234]

---

## 🎓 About You

### Age / Grade Level *
Select the option that best fits:
- [ ] **Under 13** - Elementary/Middle School
- [ ] **13-17** - High School
- [ ] **18+** - Adult/Professional
- [ ] **Parent/Guardian** - Interested in program for student

### Educational Background
What's your current education level?
- [ ] High School (or currently attending)
- [ ] Some College / Technical Training
- [ ] College Graduate (Bachelors)
- [ ] Advanced Degree (Masters/PhD)
- [ ] Other / Prefer not to say

---

## 🚀 Program Interest

### Which program interests you most? *
Choose all that apply:
- [ ] **Automation Technology** - Learn robotics, control systems, and industrial automation
- [ ] **Advanced Manufacturing** - Master CNC (Computer Numerical Control) machining, 3D printing, and production techniques
- [ ] **CNC (Computer Numerical Control) Programming** - Hands-on CAM/CAD programming for precision parts
- [ ] **Drone Operations & AI** - Explore unmanned systems and autonomous technology
- [ ] **Still exploring** - Tell us more in "Additional Comments" below

### Your Experience Level *
How familiar are you with automation/manufacturing tech?
- [ ] **Complete beginner** - Never used these tools
- [ ] **Some exposure** - Seen or tried once/twice
- [ ] **Hands-on experience** - Regular use or technical background
- [ ] **Professional** - Working in the field

---

## 🎤 Communication Preferences

### Preferred Language *
- [ ] English
- [ ] Spanish
- [ ] Arabic
- [ ] Other: [specify language]

### How did you hear about us? *
_This helps us understand which events and channels are most effective_
- [ ] Boys & Girls Club Event (Feb 3-4, 2026) - Detroit
- [ ] School / Teacher Referral
- [ ] Community Event or Workshop
- [ ] Social Media (Instagram, Facebook, TikTok)
- [ ] Word of Mouth / Friend Recommendation
- [ ] Website or Online Search
- [ ] Other: [describe where you found us]

---

## 💭 Your Interest & Goals

### Tell Us About Your Interest *
[Why are you interested in this program? Examples: career change, exploring a hobby, student project, business idea, skill-building, etc.]

### Additional Comments (Optional)
[Any other questions or information you'd like to share? We're here to help!]

---

## 📬 Next Steps (Optional)

Help us serve you better:
- [ ] I'd like to attend a free **info session** before enrolling
- [ ] Add me to the **mailing list** for program updates and event invitations
- [ ] I have **accessibility needs** - we want to make sure you're supported

---

## ⏱️ What Happens Next

Thank you for your interest! Here's what to expect:

1. ✅ **Form Submitted** — You're here now!
2. 📧 **Confirmation Email** — We'll send details within **1 hour**
3. 📞 **We'll Reach Out** — Call/text to introduce ourselves (within **24-48 hours**)
4. 📅 **Schedule Info Session** — Choose a time that works for you
5. 🎓 **Enroll & Get Started** — Begin your automation journey!

**Questions before submitting?**
📧 Email: dbkrsmith+DAA@gmail.com
📞 Call/Text: (313) 306-3767
🌐 Website: detroitautomationacademy.org

---

*Thank you for applying to Detroit Automation Academy!*
