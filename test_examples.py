...
Tests cover Phase 1 (GPIO patterns), Phase 2 (CAD generation),
and Phase 3 (rover simulation).
"""
...
class TestPhase1GPIO:
    ...
    def test_pull_up_resistor_configuration(self):
        ...
        print(
            f"✓ Pull-up configuration: default={pin_state_default}, trigger={edge_type}"
        )
...
class TestPhase2CAD:
    ...
    def test_parametric_dimensions(self):
        ...
        assert (
            length > width
        ), "Chassis should be longer than wide (typical rover shape)"
        print(
            f"✓ Rover chassis parameters: {length}mm long × {width}mm wide × {height}mm tall"
        )
    ...
    def test_stl_file_generation(self):
        ...
        assert hasattr(
            cad_design, "STLWriter"
        ), "cad_design module should have STLWriter class"
        assert hasattr(
            cad_design, "create_rover_chassis"
        ), "cad_design should have create_rover_chassis function"
...
class TestPhase3Rover:
    ...
    def test_rover_initialization(self):
        ...
        print(
            f"✓ Rover initialization: default (0,0) facing North, custom positions work"
        )
    ...
    def test_world_initialization(self):
        ...
        print(
            f"✓ World initialized: {world.width}×{world.height} with {len(world.obstacles)} obstacles"
        )
    ...
    def test_world_boundary_checking(self):
        ...
        assert not world.is_valid_position(
            10, 5
        ), "x=10 should be invalid (10×10 grid is 0-9)"
        assert not world.is_valid_position(
            5, 10
        ), "y=10 should be invalid (10×10 grid is 0-9)"
...
class TestPhase3Performance:
    ...
    def test_rover_multiple_moves(self):
        ...
        distance = abs(final_pos[0] - initial_pos[0]) + abs(
            final_pos[1] - initial_pos[1]
        )
        assert distance > 0, "Rover should have moved"

        print(
            f"✓ Performance: 1000 moves completed (start={initial_pos}, end={final_pos}, distance={distance})"
        )
...
class TestPhase2STLGeneration:
    ...
    def test_stl_writer_creates_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(
                f"✓ STL file generation: {filepath} ({os.path.getsize(filepath)} bytes)"
            )
    ...
    def test_rover_chassis_generation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                assert (
                    file_size > 300
                ), f"Rover chassis should have geometry (got {file_size} bytes)"
            ...
            try:
                ...
                for radius in [2, 3, 5]:
                    ...
                    assert os.path.exists(
                        filepath
                    ), f"Sensor mount STL should exist for radius={radius}"
                    assert (
                        os.path.getsize(filepath) > 500
                    ), f"Sensor mount should have geometry"
...
class TestPhase2GCodeValidation:
    ...
    def test_gcode_syntax_validation(self):
        ...
        for line in valid_gcode_lines:
            ...
            assert any(
                line.startswith(cmd) for cmd in ["G", "M"]
            ), f"G-code should start with G or M: {line}"
            assert any(
                char.isdigit() for char in line
            ), f"G-code should have numeric code: {line}"
    ...
    def test_material_compatibility_parameters(self):
        ...
        for material_name, power, speed in test_params:
            ...
            assert (
                speed < 400
            ), f"{material_name} speed should be reasonable (<400 mm/min)"
...
class TestPhase3WorldObstacles:
    ...
    def test_world_with_obstacles(self):
        ...
        assert not world.is_valid_position(
            5, 5
        ), "Position with obstacle should be invalid"
    ...
    def test_rover_obstacle_avoidance_strategy(self):
        ...
        ), "Obstacle should block eastward movement"
    ...
    def test_multiple_obstacles_maze(self):
        ...
        for pos in corridor_positions:
            assert world.is_valid_position(
                pos[0], pos[1]
            ), f"Corridor position {pos} should be valid"
        ...
        print(
            f"✓ Maze navigation: {len(world.obstacles)} obstacles created, corridor validated"
        )
    ...
    def test_mock_button_press_event(self):
        ...
        class MockGPIOEvent:
            ...
            def add_event_detect(pin, edge, callback=None):
                MockGPIOEvent.events.append(
                    {"pin": pin, "edge": edge, "callback": callback}
                )
